...
def load_model(directory, is_gpu=True, filename=MODEL_FILENAME, whittington=0):
    ...
    path_to_model = os.path.join(directory, filename)
    model = _get_model(model_type, img_size, latent_dim, device, path_to_model, whittington)
    return model
...
def _get_model(model_type, img_size, latent_dim, device, path_to_model, whittington=0):
    """ Load a single model.
    ...
    """
    model = init_specific_model(model_type, img_size, latent_dim, whittington).to(device)
