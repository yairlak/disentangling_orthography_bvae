...
# SPECS
loss='betaB'
dataset='dwords' # dletters/dwords
    ...
    for BATCH_SIZE in $BATCH_SIZES; do
        for LATENT_SIZE in $LATENT_SIZES; do
            for LEARNING_RATE in $LEARNING_RATES; do
                ...
                cmd='python main.py '$loss'_'$dataset'_'$name' --epochs '$EPOCHS' --dataset '$dataset' --experiment custom --eval-batchsize '$BATCH_SIZE' --betaH-B '$BETA' --batch-size '$BATCH_SIZE' --latent-dim '$LATENT_SIZE' --lr '$LEARNING_RATE' --no-progress-bar --is-metrics'
                echo $cmd
                eval $cmd
            done
        done
