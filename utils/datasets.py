...
################################ NEW DATASET ADDED ###################
class Letters(datasets.ImageFolder):  

    files = {"train": "train"}
    img_size = (3, 64, 64)
    background_color = COLOUR_WHITE

    def __init__(self, root=os.path.join(DIR, '../data/letters'),
                 logger=logging.getLogger(__name__)):
        self.root = root
        self.train_data = os.path.join(root, type(self).files["train"])
...
class Dletters(DisentangledDataset):

    files = {"train": "dletters.npz"}
    lat_names = ('shape', 'scale', 'posX', 'posY','font','upper')
    img_size = (1, 64, 64)
    background_color = COLOUR_BLACK
    lat_values = {'shape': np.array(np.arange(26)),
                  'scale': np.array([15, 18, 21, 24, 27, 30]),
                  'posX': np.arange(-8, 8, 1),
                  'posY': np.arange(-8, 8, 1),
                  'font': np.array([0,1,2]),
                  'upper': np.array([0,1]),
                  'color': np.array([1.])}
